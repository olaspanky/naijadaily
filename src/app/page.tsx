...
//   const [totalItems, setTotalItems] = useState(0); // Store total items from API
//   const limit = 10; // Fixed items per page, aligned with backend default
...
//   // Fetch news from API for the current page
//   useEffect(() => {
//     const fetchNews = async () => {
//       try {
//         // Construct API URL with pagination and optional category filter
//         let apiUrl = `https://news-app-three-lyart.vercel.app/news-app/published?page=${currentPage}&limit=${limit}`;
...
//     >
      ...
      //       ? `${featuredNews[0].title.substring(0, 60)} | Naija Daily`
      //       : defaultTitle}
      //   </title>
      //   <meta
      //     name="description"
      //     content={
      //       featuredNews.length > 0
      //         ? featuredNews[0].excerpt.substring(0, 160)
      //         : defaultDescription
      //     }
      //   />
      //   <meta
      ...
      //     content={
      //       featuredNews.length > 0
      //         ? featuredNews[0].title.substring(0, 60)
      //         : defaultTitle
      //     }
      //   />
      //   <meta
      ...
      //     content={
      //       featuredNews.length > 0
      //         ? `Featured image for ${featuredNews[0].title}`
      //         : "Naija Daily logo"
      //     }
      //   />
      ...
      //     name="twitter:image"
      //     content={
      //       featuredNews.length > 0 ? featuredNews[0].image : defaultImage
      //     }
      //   />
      //   <meta
...
//                 <p className="mb-2">
//                   Suite 4, Block A, G-Wing, Bassan Plaza, Off Herbert Macaulay Way,
...
import Head from "next/head";
import SkeletonLoader from "../app/components/SkeletonLoader"; // Import the new component
...
export default function DailyPostClone() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      setIsLoading(true); // Set loading to true before fetching categories
      try {
        const response = await fetch(
          "https://news-app-three-lyart.vercel.app/news-app-category"
        );
  ...
  // Update selectedCategory handler to reset page and trigger loading
  ...
  return (
    ...
    >
      ...
      <Navbar
        categories={categories}
        selectedCategory={selectedCategory}
        setSelectedCategory={handleCategoryChange} // Use updated handler
        darkMode={darkMode}
        toggleDarkMode={toggleDarkMode}
      />
      ...
      >
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex items-center">
            ...
            <div className="overflow-hidden whitespace-nowrap flex-1" aria-live="polite">
              {isLoading ? (
                <div className="h-4 w-3/4 bg-gray-300 dark:bg-gray-700 animate-pulse rounded"></div>
              ) : featuredNews.length > 0 ? (
                <div className="inline-block animate-marquee">
                  <span className="inline-block pr-8">
                    {featuredNews.slice(0, 4).map((news) => news.title).join(" • ")}
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          ...
          <div className="w-full lg:w-2/3">
            <section className="mb-8">
              ...
              {isLoading ? (
                <SkeletonLoader darkMode={darkMode} count={4} />
              ) : featuredNews.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {featuredNews.slice(0, 4).map((news) => (
                    <article
